...
# based so you must enter a valid: username, password, and client id. Optionally, you may specify a file type
...
function cognito_create_user_tokens () {
    ...
    echo -n "Enter the file name to save the tokens to (leave blank to skip): "
    read -e file_name
    ...
    if [[ -n "$file_name" ]]; then
        echo "Check $file_name for your tokens."
        {
